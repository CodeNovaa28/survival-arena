...
export interface GunDef {
  ...
  bulletSpeed: number; // units/second
  range: number;       // bullet lifetime in seconds
  bulletCount: number; // bullets per shot
  spread: number;      // spread half-angle in degrees (for multi-shot)
  ...
  // Stat bar values (0-10 for display)
  statFireRate: number;
  statDamage: number;
  statRange: number;
}
...
export const GUNS: GunDef[] = [
  {
    id: "pistol",
    name: "Pistol",
    description: "Standard sidearm. Slow rate of fire, limited range.",
    cost: 0,
    tier: "basic",
    fireRate: 0.55,
    damage: 22,
